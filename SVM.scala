...
val dataf = spark.read.option("header", "true").option("inferSchema","true").option("delimiter",";")csv("bank.csv")
...
val pipeline = new Pipeline().setStages(Array(labelIndexer, featureIndexer, lsvcModel, labelConverter))
...
println("Duration Seconds: " + (endTimeMillis - startTimeMillis))